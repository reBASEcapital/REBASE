...
library UniswapV2OracleLibrary {
    ...
    function currentPrice(
        address pair,
        bool isToken0
    ) internal view returns (uint price, uint32 blockTimestamp) {

        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = UniswapPair(pair).getReserves();
        if (isToken0) {
                price = uint(FixedPoint.fraction(reserve1, reserve0)._x) ;
        } else {
                price = uint(FixedPoint.fraction(reserve0, reserve1)._x);
        }
